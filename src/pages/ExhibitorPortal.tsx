...
const services = [
  { name: 'Electricidad 220V', status: 'Contratado' },
  { name: 'Conexión internet fibra', status: 'Contratado' },
  { name: 'Limpieza diaria', status: 'Incluido' },
  { name: 'Montaje estándar', status: 'Pendiente confirmación' },
  { name: 'Badge exposiciones (x4)', status: 'Confirmado' },
];

const dates = [
  { label: 'Montaje', date: '19-20 Ene 2026', status: 'Pendiente' },
  { label: 'Apertura', date: '21 Ene 2026', status: 'Confirmada' },
  { label: 'Clausura', date: '25 Ene 2026', status: 'Confirmada' },
  { label: 'Desmontaje', date: '26 Ene 2026', status: 'Pendiente' },
];
...
export default function ExhibitorPortal() {
  ...
  const [localRequests, setLocalRequests] = useState<Reservation[]>([]);

  const exhibitorRequests = useMemo(() => {
    if (!fair || !assignment) return [];

    const fromMock = reservations.filter(
      reservation => reservation.fairId === fair.id && reservation.company === assignment.companyName
    );

    return [...localRequests, ...fromMock];
  }, [localRequests, fair, assignment]);
  ...
  const availableToRequest = fairStands
    .filter(stand => ["available", "proposed", "pending"].includes(stand.status))
    .sort((a, b) => a.code.localeCompare(b.code));

  const requestReservation = (standId: string) => {
    if (!fair || !assignment) return;
    ...
    const newRequest: Reservation = {
      ...
      standId: stand.id,
      fairId: fair.id,
      fairName: `${fair.name} ${fair.edition}`,
      company: assignment.companyName,
      requester: activeUser.name,
      requesterRole: "exhibitor",
      date: new Date().toISOString().slice(0, 10),
      status: "solicitud",
      validators: [assignment.commercialContact.name],
      comments: "Solicitud enviada desde portal de expositor",
    };
      ...
      <div className="max-w-3xl mx-auto space-y-4 animate-fade-in">
        <div className="bg-card rounded-lg border border-border p-6">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Esta vista está disponible solo para usuarios con rol expositor.
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-4 animate-fade-in">
        <div className="bg-card rounded-lg border border-border p-6">
          <h1 className="text-xl font-semibold text-foreground">Exhibitor Portal</h1>
          <p className="text-sm text-muted-foreground mt-2">
            No hay una feria asignada para este expositor. Contacta con administración para activar tu acceso.
          </p>
        </div>
      </div>
    );
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="bg-card rounded-lg border border-border p-6">
        <div className="flex items-start justify-between">
          ...
          <StatusBadge status={exhibitorReservation?.status || 'solicitud'} type="reservation" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-card rounded-lg border border-border p-5">
            ...
            {!exhibitorStand ? (
              <p className="text-sm text-muted-foreground">Todavía no tienes un stand principal asignado.</p>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { icon: Tag, label: 'Código', value: exhibitorStand.code },
                { icon: Ruler, label: 'Superficie', value: `${exhibitorStand.area} m²` },
                { icon: MapPin, label: 'Zona', value: exhibitorStand.zone },
                { icon: Package, label: 'Tipo', value: exhibitorStand.type },
              ].map(item => (
                <div key={item.label}>
          ...
          <div id="plano" className="bg-card rounded-lg border border-border p-5">
            <h2 className="font-semibold text-foreground mb-4">Ubicación en plano</h2>
          ...
          <div className="bg-card rounded-lg border border-border p-5">
            <h2 className="font-semibold text-foreground mb-4">Solicitar reservas de stand</h2>
            {availableToRequest.length === 0 ? (
              <p className="text-sm text-muted-foreground">No hay stands disponibles para solicitar en este momento.</p>
            ) : (
                ...
                {availableToRequest.slice(0, 12).map(stand => {
                  ...
                  return (
                    <div key={stand.id} className="flex items-center justify-between gap-2 py-2 border-b border-border last:border-0">
                      ...
                      <button
                        onClick={() => requestReservation(stand.id)}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card rounded-lg border border-border p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-semibold text-foreground">Documentación</h2>
            </div>
            <div className="space-y-2">
              {[
                'Manual del expositor',
                `Plano vigente ${currentVersion?.label || ''}`.trim(),
                'Normas técnicas de montaje',
          ...
          <div className="bg-status-pending/10 border border-status-pending/30 rounded-lg p-4">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-xs font-medium text-foreground">Acción requerida</p>
                <p className="text-xs text-muted-foreground mt-1">Confirmar servicio de montaje antes del 15 de enero.</p>
              </div>
            </div>
          </div>
        </div>
