...
export default function VersionsList() {
  ...
  const publishedVersions = groupedFairs.reduce(
    (acc, item) => acc + item.versions.filter(version => version.status === "published").length,
    0
  );
  const draftVersions = groupedFairs.reduce(
    (acc, item) => acc + item.versions.filter(version => version.status === "draft").length,
    0
  );
  const commercialDraftVersions = groupedFairs.reduce(
    (acc, item) => acc + item.versions.filter(version => version.status === "commercial_draft").length,
    0
  );

  return (
    <div className="space-y-6 animate-fade-in">
      <Breadcrumbs items={[{ label: "Dashboard", href: "/" }, { label: "Versions" }]} />

      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Version management</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {totalVersions} versions across {groupedFairs.length} fairs, sorted by fair.
          </p>
        </div>
        ...
        <ListFilters
          ...
          searchPlaceholder="Search fair, version or responsible"
          searchAriaLabel="Search versions"
          dateFrom={dateFrom}
          dateTo={dateTo}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
        <div className="bg-card rounded-lg border border-border p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">Fairs with versions</p>
          <p className="text-2xl font-bold text-foreground mt-1">{groupedFairs.length}</p>
        </div>
        <div className="bg-card rounded-lg border border-border p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">Total versions</p>
        ...
        <div className="bg-card rounded-lg border border-border p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">Commercial drafts</p>
          <p className="text-2xl font-bold text-foreground mt-1">{commercialDraftVersions}</p>
        </div>
      </div>
          ...
          <section key={fair.id} className="bg-card rounded-lg border border-border overflow-hidden">
            <div className="p-4 border-b border-border flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  {can("manage_versions") ? "Manage versions" : "View versions"}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/40">
                    {[
                      { label: 'Version', key: 'label',     mode: 'text' as const, opts: undefined,                                     lbls: undefined },
                      { label: 'Status',  key: 'status',    mode: 'enum' as const, opts: Object.keys(fairVersionStatusLabels),           lbls: fairVersionStatusLabels as Record<string,string> },
                      { label: 'Date',   key: 'createdAt', mode: null,            opts: undefined,                                     lbls: undefined },
                      { label: 'Author', key: 'createdBy', mode: 'text' as const, opts: undefined,                                     lbls: undefined },
                      { label: 'Summary', key: 'summary',   mode: 'text' as const, opts: undefined,                                     lbls: undefined },
                    ].map(col => (
                      <SortableHeader
                        key={col.key}
                        label={col.label}
      ...
      {groupedFairs.length === 0 && (
        <div className="bg-card rounded-lg border border-border p-8 text-center">
          ...
          <h2 className="text-lg font-semibold text-foreground mt-3">No versions</h2>
          <p className="text-sm text-muted-foreground mt-1">
            No fair versions to display with the current filters.
          </p>
        </div>
      )}
    </div>
