...
import { users, roleLabels, fairs, reservations, type User } from "@/data/mockData";
...
export default function FairUsers() {
  ...
  const { sortedItems: sortedFairUsers, sortKey, sortDir, handleSort } = useTableSort(columnFiltered, (item, key) => {
    switch (key) {
      ...
      case 'fairs': return (associatedFairsByUser.get(item.id) || []).length;
      default:     return null;
    }
  });
        ...
        <div className="overflow-x-auto">
          ...
          <thead>
            <tr className="border-b border-border bg-muted/50">
              {[
                ...
                { label: 'Associated fairs', key: 'fairs', mode: null,            opts: undefined,               lbls: undefined },
              ].map(col => (
                <SortableHeader
                  key={col.key}
                  label={col.label}
          ...
          <tbody className="divide-y divide-border">
            {paginatedFairUsers.map((u, index) => {
              ...
              const PermIcon = perm.icon;
              const associatedFairs = associatedFairsByUser.get(u.id) || [];
              return (
                <motion.tr
                  key={u.id}
                  initial={{ opacity: 0, x: -10 }}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1.5">
                      {associatedFairs.length > 0 ? (
                        associatedFairs.slice(0, 2).map(fair => (
                          <Link key={fair.id} to={`/fairs/${fair.id}`} className="px-2 py-1 bg-muted rounded text-[10px] text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors">
                            {fair.name}
                          </Link>
                        ))
                      ) : (
