  ...
  const contextCard = useMemo(() => {
    ...
    if (pathname.startsWith("/fairs/")) {
      return {
        ...
        lines: [
          ...
          fairFromRoute ? `Fair: ${fairFromRoute.name} ${fairFromRoute.edition}` : "Fair: unidentified",
          currentVersion ? `Version: ${currentVersion.label}` : undefined,
        ].filter(Boolean) as string[],
      };
    }
    ...
    if (pathname.startsWith("/venues/")) {
      return {
        title: "Venue detail",
        section: "Hierarchy",
        lines: [venueFromRoute ? `Venue: ${venueFromRoute.name}` : "Venue: unidentified"],
      };
    }
