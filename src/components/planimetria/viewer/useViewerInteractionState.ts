...
export function useViewerInteractionState({
  ...
  const [editMode, setEditMode] = useState<'camera' | 'select' | 'move' | 'shape' | 'cut' | 'merge' | 'duplicate' | 'walls'>('select');
  ...
  useEffect(() => {
    ...
    // Exit walls mode automatically if selection is no longer a stand
    if (editMode === 'walls' && focusTarget?.type !== 'stand') {
      setEditMode('select');
    }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Exit walls mode with Escape
      if (e.key === 'Escape' && editMode === 'walls') {
        setEditMode('select');
        return;
      }
      ...
      // W — activate walls tool when a stand is selected
      if (e.key.toLowerCase() === 'w' && focusTarget?.type === 'stand' && canUseTools && !e.ctrlKey && !e.metaKey) {
        setEditMode('walls');
        e.preventDefault();
      }
    };
