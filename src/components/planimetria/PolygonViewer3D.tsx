...
import { Camera, Grid3x3, Magnet } from 'lucide-react';
import { requestReservation, reservations, roleLabels, setReservationStatus } from '@/data/mockData';
...
function StandDuplicateArrow3D({ position, side, onClick }) {
  ...
  return (
    ...
    >
      <mesh rotation={[-Math.PI / 2, 0, 0]} renderOrder={1100}>
      ...
      <mesh position={[0, 0.02, 0.05]} renderOrder={1101}>
        <boxGeometry args={[0.14, 0.05, 0.62]} />
        <meshBasicMaterial color={hovered ? '#fbcfe8' : '#f9a8d4'} depthTest={false} />
      </mesh>

      <mesh position={[0, 0.02, 0.48]} rotation={[Math.PI / 2, 0, 0]} renderOrder={1101}>
        <coneGeometry args={[0.2, 0.32, 12]} />
        <meshBasicMaterial color={hovered ? '#fbcfe8' : '#f9a8d4'} depthTest={false} />
      </mesh>
    </group>
  );
...
export default function PolygonViewer3D({ onBack, fairId = 'f1' }) {
  ...
  const handleReserveSelectedStand = async () => {
    if (!selectedStand || !canManageReservations) return;
  ...
  const handleReleaseSelectedStand = async () => {
    ...
    const activeReservation = [...reservations]
      .reverse()
      .find((reservation) => reservation.standId === selectedStand.id && ['solicitud', 'pendiente', 'en_revision', 'aprobada'].includes(reservation.status));
    ...
    setFloorPlanError('');
    setReservationStatus(activeReservation.id, 'cancelada');
    setStands((prev) =>
      prev.map((stand) =>
        stand.id === selectedStand.id
          ? {
  ...
  const handleQuickApproveReservation = (reservationId) => {
    ...
    setReservationStatus(reservation.id, 'aprobada');
    setStands((prev) =>
      prev.map((stand) => {
  ...
  const handleQuickCancelReservation = (reservationId) => {
    ...
    setReservationStatus(reservation.id, 'cancelada');
    setStands((prev) =>
      prev.map((stand) => {
        const matchesStand = stand.fairStandId
          ? String(stand.fairStandId) === String(reservation.standId)
  ...
  return (
    <div className="polygon-viewer-container">
      <div className="polygon-viewer-content">
        ...
        <div className="viewer-superior-toolbar" aria-label="SUPERIOR">
          <button
